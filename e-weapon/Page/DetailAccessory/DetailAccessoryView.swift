...
struct DetailAccessoryView: View {
    ...
    @StateObject var viewModel: DetailAccessoryViewModel = DetailAccessoryViewModel()
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            VStack(spacing: 10) {
                Group {
                    TitleSubForm(title: "Accessories Information")
                        .hLeading()
        ...
        .navigationTitle("Detail Accessory")
                ...
                Button {
                    if isEdit {
                        ...
                        if let price = Double(self.price), let stock = Int(self.stock),  let image = self.currentImage {
                            ...
                            viewModel.updateAccessory(id: self.id, name: self.name, addedAt: Date(), price: price, stock: stock, location: self.locationSelected, status: self.statusSelected, imageUrl: imageUrl, image: image) { result in
                                switch(result){
                                case .success:
                                    showAlert(isActive: true, title: "Success", message: "Accessory has been updated")
                                case .failure(_):
...
struct DetailAccessoryView_Previews: PreviewProvider {
    static var previews: some View {
        DetailAccessoryView(id: "", imageUrl: "", addedAt: Date())
    }
}
